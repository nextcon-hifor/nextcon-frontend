  ...
  <div class="chat-container">
    ...
    <div class="chat-sidebar">
      ...
      <div v-else class="chat-room-list">
        <div v-for="room in chatRooms" :key="room.id" class="chat-room-item"
          ...
          <div class="room-meta">
            <div class="message-time">{{ formatTime(room.lastMessageTime) }}</div>
            <div v-if="room.unreadCount" class="unread-count">{{ room.unreadCount }}</div>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="chat-messages-container" v-if="currentChatId">
      ...
      <div class="message-input-container">
        <input v-model="newMessage" type="text" placeholder="Type a message..." @keyup.enter="sendNewMessage"
...
socket.on('chat:message', (message) => {
  // If the message is for the current chat, add it
  if (message.chatId === currentChatId.value) {
    ...
    // 기존 markChatAsRead 호출 제거 
    // 현재 채팅방의 메시지라면 UI에서 읽음으로 표시
    const room = chatRooms.value.find(r => r.id === message.chatId);
    if (room) {
      room.unreadCount = 0;
    }
  } else {
    // Otherwise, update the unread count for that chat
    const room = chatRooms.value.find(r => r.id === message.chatId);
    if (room) {
...
watch(() => connectionStatus.connected, (isConnected) => {
  if (isConnected && currentChatId.value) {
    // Reconnect to the current room if we were disconnected
