                ...
                <form class="search-form" @submit.prevent="searchEvents">
                    <div class="row search-bar">
                        ...
                        <div class="col-6 search-text">
                            <button type="submit">
                                <img
                                    src="/assets/img/icons_search.png"
                                    alt=""
                                />
                            </button>
                            <input
...
const categories = ref([
    { name: "All", displayName: "ALL", icon: "/assets/img/icon_All.png" },
    {
        name: "Social",
        displayName: "SOCIAL",
        icon: "/assets/img/icon_Social.png",
    },
    ...
    { name: "Games", displayName: "GAMES", icon: "/assets/img/icon_Games.png" },
    {
        ...
        icon: "/assets/img/icon_Growth.png",
    },
    {
        name: "Activities",
        displayName: "ACTIVITIES",
        icon: "/assets/img/icon_Sports.png",
    },
    { name: "Trip", displayName: "TRIP", icon: "/assets/img/icon_Trip.png" },
    {
        name: "Art/Fasion",
        displayName: "ART/FASHION",
        icon: "/assets/img/icon_ArtFasion.png",
    },
    {
        name: "Others",
        displayName: "OTHERS",
        icon: "/assets/img/icon_Others.png",
    },
]);
...
const mapEventData = (event) => {
    // 이벤트 날짜와 시간을 결합하여 Date 객체 생성
    const eventDateTime = new Date(`${event.date}T${event.time}`);
    const currentDateTime = new Date();

    // 이벤트가 이미 종료되었는지 확인
    if (eventDateTime < currentDateTime) {
        return null; // 종료된 이벤트는 null 반환
    }
...
const fetchEvents = async (fetchType, params = {}) => {
    try {
        ...
        return response.data
            .map(mapEventData)
            .filter((event) => event !== null); // 데이터 매핑 후 반환
    } catch (error) {
...
@media screen and (min-width: 769px) {
    ...
    .search-date input {
        background: url("/assets/img/icon_SearchDate.png") no-repeat right 5px
            center;
    }
