    ...
    <div class="create-form">
      <form @submit.prevent="postEvent">
        ...
        <div class="form-group">
          ...
          <p>The first image will be the main image of the event. (Up to 5 photos)</p>

          <!-- 파일 입력 -->
          <input type="file" id="file-upload" multiple accept="image/*" @change="handleFileUpload" style="display: none" />

          <!-- 클릭 가능한 업로드 박스 -->
          <div id="upload-box" @click="triggerFileInput" @dragover.prevent @drop.prevent="handleDrop" style="border: 2px dashed #ccc; padding: 20px; text-align: center; cursor: pointer">
          ...
          <ul id="file-list" style="margin-top: 10px; list-style: none; padding: 0">
            <li v-for="(file, index) in uploadedFiles" :key="index">
              ...
              {{ file.name }}
              <button @click="removeFile(index)" style="margin-left: 10px; cursor: pointer; color: red">Remove</button>
            </li>
          </ul>
        </div>
        <!-- Dropzone 끝 -->
        ...
        <div class="form-group">
          <div class="row half-input-row">
            <div class="col-6">
              <label for="">Number of participants (Minimun)</label>
              <input type="number" v-model="form.minParticipants" placeholder="Minimum" required />
            </div>
            <div class="col-6">
              <label for="">Number of participants (Maximum)</label>
        ...
        <div class="form-group">
          ...
          <textarea class="ipnut-question" id="editorTxt" placeholder="Where are you from?" v-model="form.question"></textarea>
        </div>
        ...
        <div class="login-container-op2">
          <div class="create-form-op2">
            <div class="agreement-container">
              <!-- 체크박스와 텍스트 -->
              <label class="agreement-label">Notice: Once an event post is published, it cannot be edited</label>
              <label class="agreement-label">I agree to include all additional fees in the event details and understand that failing to do so may result in penalties</label>
...
const postEvent = async () => {
  try {
    ...
    for (const fileObj of uploadedFiles.value) {
      ...
      const uploadResponse = await axios.post(`${import.meta.env.VITE_API_BASE_URL}/events/upload-image-postEvent`, formData, {
    ...
    const mainImage = uploadedImageUrls.length > 0 ? uploadedImageUrls[0] : null; // 첫 번째 파일
    const images = uploadedImageUrls.length > 1 ? uploadedImageUrls.slice(1) : []; // 나머지 파일들
    const eventData = {
    ...
    try {
      ...
      const response = await axios.post(`${import.meta.env.VITE_API_BASE_URL}/events/submit`, enrichedFormData, {
        withCredentials: true, // 인증 정보를 포함
      });
...
const openPopup = () => {
  ...
  const popupWindow = window.open('', 'Usage Rules', 'width=600,height=700,scrollbars=yes,resizable=yes');
